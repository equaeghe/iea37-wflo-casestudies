...
def rose_power(turb_coords, downwind_vectors, wind_speed, turb_ci, turb_co):
    ...
    for i, downwind in enumerate(downwind_vectors):
        ...
        wake_deficit, blame_fractions = GaussianWake(frame_coords)
        # Calculate powers
        powers[i, :] = power(wake_deficit, wind_speed, turb_ci, turb_co)

    return powers, blame_fractions
...
def pseudo_gradient(wind_freq, turbine_vectors, deficits, blame_fractions):  # TODO: make use of blame_fraction; requires pairwise unit vectors between turbines instead of downwind vectors
