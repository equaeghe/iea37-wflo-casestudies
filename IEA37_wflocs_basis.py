...
def power(turb_coords, downwind, wind_speed, turb_ci, turb_co):
...
def rose_power(turb_coords, downwind_vectors, wind_speed, turb_ci, turb_co):
    ...
    powers = np.zeros(shape=(len(downwind_vectors), len(turb_coords)),
                      dtype=np.double)
    for i, downwind in enumerate(downwind_vectors):
        powers[i, :] = power(turb_coords, downwind, wind_speed,
                             turb_ci, turb_co)

    return powers


def rose_deficits(wind_speed, turb_ci, turb_co, powers):
    """Calculate the power deficits of each turbine for every direction"""
    # powers is the output of rose_power
    ...
    return wakeless_pwr - powers


def pseudo_gradient(wind_freq, downwind_vectors, deficits):
    """Calculate the pseudo gradient for each turbine"""
    # deficits is the output of rose_deficits
    pseudo_gradient = np.recarray((deficits.shape[1],), coordinate)
    pseudo_gradient.x = np.sum((wind_freq * downwind_vectors.x) * deficits.T,
                               axis=1)
    pseudo_gradient.y = np.sum((wind_freq * downwind_vectors.y) * deficits.T,
                               axis=1)
